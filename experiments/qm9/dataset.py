...
class QM9(InMemoryDataset):
    ...
    def __init__(
        self, root, target, radius, partition, lmax_attr, feature_type="one_hot"
    ):
    ...
    def processed_file_names(self) -> List[str]:
        return [
            "_".join(
                [
                    self.partition,
                    "r=" + str(np.round(self.radius, 2)),
                    self.feature_type,
                    "l=" + str(self.lmax_attr),
                ]
            )
            + ".pt"
        ]
    ...
    def process(self):
        ...
        print(
            "Processing",
            self.partition,
            "with radius=" + str(np.round(self.radius, 2)) + ",",
        ...
        for i, mol in enumerate(tqdm(suppl)):
            ...
            edge_attr, node_attr, edge_dist = self.get_O3_attr(
                edge_index, pos, self.attr_irreps
            )
            ...
            data = Data(
                ...
                edge_attr=edge_attr,
                node_attr=node_attr,
                additional_message_features=edge_dist,
                y=y,
                name=name,
                index=i,
            )
    ...
    def get_O3_attr(self, edge_index, pos, attr_irreps):
        ...
        edge_attr = spherical_harmonics(
            attr_irreps, rel_pos, normalize=True, normalization="component"
        )  # Unnormalised for now
    ...
    def top_n_nodes(self, n: int) -> List[int]:
        ...
        return [int(k) for k in torch.topk(torch.diff(self.slices["x"]), n)[0]]

    def top_n_edges(self, n: int) -> List[int]:
        """Returns the largest n edge in the dataset."""
        return [int(k) for k in torch.topk(torch.diff(self.slices["edge_attr"]), n)[0]]
...
def get_cormorant_features(one_hot, charges, charge_power, charge_scale):
    ...
    charge_tensor = (charges.unsqueeze(-1) / charge_scale).pow(
        torch.arange(charge_power + 1.0, dtype=torch.float32)
    )
...
if __name__ == "__main__":
    dataset = QM9("datasets", "alpha", 2.0, "train", 3, feature_type="one_hot")
